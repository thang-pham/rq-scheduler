#!/usr/bin/env bash
# Setup system to host RQ scheduler.
...
function configure_centos {
    ...
    cp $RQ_DIR/etc/supervisor/conf.d/rqscheduler.conf $supervisor_conf

    # rqscheduler is found under /usr/bin
    sed -i 's#/local##g' $supervisor_conf/rqscheduler.conf
}
...
function switch_localhost {
    ...
    sed -i 's/localhost/redis/g' /etc/supervisor/conf.d/rqscheduler.conf
    sed -i 's/localhost/redis/g' /etc/supervisord.d/rqscheduler.conf
}
...
if [[ $1 == "-h" ]]; then
    echo "Usage: $0 [--docker]"
    echo ""
    echo "Setup system to host RQ scheduler."
